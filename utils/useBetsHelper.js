...
export default function (fixture) {
    ...
    const getGoals = (acc, stats, value, time, index, matchesLength, isH2H = false) => {
        ...
        if (time == 'fulltime') goalsInGame = stats.score.fulltime.home + stats.score.fulltime.away;
    ...
    const getBothTeamsToScore = (acc, stats, time, index, matchesLength, isH2H = false) => {
        ...
        if (time == 'fulltime') {
            goalsHome = stats.score.fulltime.home;
            goalsAway = stats.score.fulltime.away;
        }
        ...
        if (time == 'secondhalf') {
            goalsHome = stats.score.fulltime.home - stats.score.halftime.home;
            goalsAway = stats.score.fulltime.away - stats.score.halftime.away;
        }
    ...
    const getCleanSheet = (acc, stats, index, matchesLength, isH2H = false) => {
        const { away, home } = stats.teams;
        const homeTeamSide = Object.values(stats.teams).find((team) => team.id === homeTeam.value.id);
        const awayTeamSide = Object.values(stats.teams).find((team) => team.id === awayTeam.value.id);

        const isAwayGames = index < matchesLength / 2;
        const isGameBetweenEachOther = homeTeamSide && awayTeamSide;
        const oppositeHomeGoals = homeTeamSide?.id == home.id ? stats.score.fulltime.away : stats.score.fulltime.home;
        const oppositeAwayGoals = awayTeamSide?.id == away.id ? stats.score.fulltime.home : stats.score.fulltime.away;
        ...
        if (isGameBetweenEachOther) {
            isAwayGames ? sumToCategory(acc, 'yes', 'away', oppositeAwayGoals, awayTeamSide) : sumToCategory(acc, 'yes', 'home', oppositeHomeGoals, homeTeamSide);
        } else {
            isAwayGames ? sumToCategory(acc, 'yes', 'away', oppositeAwayGoals, awayTeamSide) : sumToCategory(acc, 'yes', 'home', oppositeHomeGoals, homeTeamSide);
        }
    };
    ...
    const toWinToNil = (acc, stats) => {
        const homeTeamSide = Object.values(stats.teams).find((team) => team.id === homeTeam.value.id);
        const awayTeamSide = Object.values(stats.teams).find((team) => team.id === awayTeam.value.id);
        let isThereNoGoal = Object.values(stats.score.fulltime).find((goal) => !!goal);
