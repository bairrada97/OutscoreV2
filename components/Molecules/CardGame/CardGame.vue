...
const { renderFixtureStatus, isGameLive } = useFixtureStatus(
  props.game,
  props.type
);
...
watch(
  ...
  (newValue, prevValue) => {
    ...
    if (
      JSON.stringify(newGoals) === JSON.stringify(prevGoals) ||
      newTime - prevTime > 2
    ) {
      return;
    }
